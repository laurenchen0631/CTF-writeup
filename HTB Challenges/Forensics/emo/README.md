...
1. First, we try `olevba` to extract macros from the `emo.doc`.
...
2. We put `emo.doc` onto the CyberChef and used `Strings` extraction with more than `250` characters. We found out a long sting with some repeating pattern. In the previous step, `Replace(Gav481k8nh28, "][(s)]w", Sxybmdt069cn)` shows that we can replace `][(s)]w`. After removing the obfuscation, we get something like `POwersheLL -windowstyle hidden -ENCOD               IAxxxxxx`. However, if we tried to decoded the encoded base64 string, it only gave us meaningless binary string.

3. After checking the macro again, we guessed that that long string probably needs more processing. I cannot run the macro on my system, and thus I decided to use [Any.Run](https://any.run/) to do dynamic analysis. Luckily, the free version gave us some similar powershell script but processed content.
...
4. Put the base64 string into CyberChef and add `From Base64`, `Decode Text` of `UTF-16LE`, and `Find/Replace` to replace `;` with `;\n`.
...
5. First, we use [PowerShell-Beautifier](https://github.com/DTW-DanWard/PowerShell-Beautifier) to make the code more readable. We can put the code directly into the powershell to deobfuscate. For example, `sv  0zx ([type]("{2}{0}{4}{3}{1}"-f 'e','rectory','syst','.io.di','m')  ) ;`.

```powershell
Set-Variable 0zx ([type]("{2}{0}{4}{3}{1}" -f 'e','rectory','syst','.io.di','m'));
Set-Variable txyseo ([type]("{0}{7}{5}{6}{4}{2}{1}{8}{3}" -f 'syste','tm','in','er','po','net.se','rvice','m.','anag'));
$nbf5tg3 = ('b9' + 'yp' + ('90' + 's'));
$vxnlre0 = $cludkjx + [char](64) + $r6r1tuy;
$ky3q0e8 = (('rq' + 'dx') + 'wo' + '5');
(Get-ChildItem variable:0zx).value::"creat`e`direc`t`ory" ($home + ((('ndp' + 'jrb') + ('e' + 'vk4n') + 'd' + 'p' + ('c' + 'cwr_2h') + 'nd' + 'p') -replace ('n' + 'dp'),[char]92));
$fn5ggmsh = (182,187,229,146,231,177,151,149,166);
$pyozgeo = (('j5f' + 'y1') + 'c' + 'c');
(variable txyseo).value::"securi`typ`r`otoc`ol" = (('tl' + 's1') + '2');
$fn5ggmsh += (186,141,228,182,177,171,229,236,239,239,239,228,181,182,171,229,234,239,239,228);
...
```powershell
Set-Variable 0zx ([type]("{2}{0}{4}{3}{1}" -f 'e','rectory','syst','.io.di','m'));
PS C:\Users\User\Desktop> "$0zx"
System.IO.Directory
```

6. After analyzing the code, we found a base64 string.
...
foreach ($a8i3ke1 in $anbyt1y) {
   ${a8`i`3ke1}.("{1}{2}{0}" -f 'ay','tocha','rarr').Invoke() |.("{2}{1}{0}{3}" -f '-','ach','fore','object') -Process { ${fn5`ggm`sh} += ([byte][char]${_} -bxor 0xdf) };
}
```

7. We just wrote a python script to xor `$fn5ggmsh` with `0xdf` and converted it back to ASCII. Then, we got the flag among the string.